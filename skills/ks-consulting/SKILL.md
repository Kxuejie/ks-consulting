...
- 想知道自己在 Case 框架、计算、brainstorm、沟通或总结中的表现哪里有问题
- 需要一次完整复盘、评分和下一轮练习方案
...
- 提供了完整 Casebook 的截图、PDF 或文字材料，希望获得这道题的标准答案
- 希望先识别题型，再严格遵循 Casebook 原框架生成完整分析
- 希望区分题目已给信息、可计算信息、信息缺口与需要自行 brainstorm 的内容
- 希望得到逐板块结论、计算过程、风险与下一步、行业认知补充或完整 HTML 报告
- 想通过阅读一份高质量标准答案来学习框架，而不是评价自己刚才的表现

`case-analyzer` 负责“这道题应该怎样完整分析”；`ks-case-review` 负责“学生刚才答得怎么样”。如果用户给的是 Casebook 材料而没有学生作答稿，路由到 `case-analyzer`；如果用户给的是面试或模拟练习的一问一答记录并要求评分，路由到 `ks-case-review`。
